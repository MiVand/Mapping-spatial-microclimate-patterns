...
temp_m <- data.frame(matrix(ncol = 8, nrow = 150)) 
...
names(temp_m)[1:8] <- c("TO_max", "TO_P99", "TO_P95", "TO_mean", "TO_range", "TO_P05", "TO_P01", "TO_min") # temp_metrics we are interested in
...
for (i in 1:150) {
  ...
  temp_m[i, 2] <- quantile(dat_t$T3[dat_t$Plot_ID == temp_m$Plot_ID[i]], probs = 0.99) - quantile(dat_t$t_macro[dat_t$Plot_ID == temp_m$Plot_ID[i]], probs = 0.99)
  ...
  temp_m[i, 4] <- mean(dat_t$T3[dat_t$Plot_ID == temp_m$Plot_ID[i]]) - mean(dat_t$t_macro[dat_t$Plot_ID == temp_m$Plot_ID[i]])
  temp_m[i, 5] <- diff(range(dat_t$T3[dat_t$Plot_ID == temp_m$Plot_ID[i]])) - diff(range(dat_t$t_macro[dat_t$Plot_ID == temp_m$Plot_ID[i]]))
  temp_m[i, 6] <- quantile(dat_t$T3[dat_t$Plot_ID == temp_m$Plot_ID[i]], probs = 0.05) - quantile(dat_t$t_macro[dat_t$Plot_ID == temp_m$Plot_ID[i]], probs = 0.05)
  temp_m[i, 7] <- quantile(dat_t$T3[dat_t$Plot_ID == temp_m$Plot_ID[i]], probs = 0.01) - quantile(dat_t$t_macro[dat_t$Plot_ID == temp_m$Plot_ID[i]], probs = 0.01)
  temp_m[i, 8] <- min(dat_t$T3[dat_t$Plot_ID == temp_m$Plot_ID[i]]) - min(dat_t$t_macro[dat_t$Plot_ID == temp_m$Plot_ID[i]])
}
...
ggplot(temp_m %>% gather(key = key, value = value, -Plot_ID)) +
